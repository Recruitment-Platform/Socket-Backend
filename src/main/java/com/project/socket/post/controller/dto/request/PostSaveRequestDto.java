...
public record PostSaveRequestDto(@NotBlank String title, @NotBlank String postContent, @NotBlank PostStatus postStatus,
                                 @NotBlank PostType postType, @NotBlank PostMeeting postMeeting) {

    public PostSaveInfo toSave(Long userId) {
        return new PostSaveInfo(title, postContent, postStatus, postType, postMeeting, userId);
    }
